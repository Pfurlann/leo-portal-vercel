...
function aplicarFiltrosHistoricos(pessoas, dataReferencia, incluirInativos) {
  ...
  try {
    ...
    pessoas.forEach(pessoa => {
      // Verificar se pessoa existia na data (passa incluirInativos para não excluir desligados)
...
function aplicarFiltrosHistoricosPeriodo(pessoas, dataInicio, dataFim, incluirInativos) {
  ...
  try {
    ...
    // Filtrar usando dataFim como referência para determinar o tipo histórico
    const pessoasNoFim = aplicarFiltrosHistoricos(pessoas, dataFim, incluirInativos);

    // Quando incluindo inativos: excluir quem foi desligado ANTES do período começar
    // (evita mostrar ex-membros de trimestres muito anteriores)
    if (incluirInativos) {
      return pessoasNoFim.filter(pessoa => {
        if (pessoa.dataDesligamento && stringValida(pessoa.dataDesligamento)) {
          const dataDesligamento = converterDataBRParaJS(pessoa.dataDesligamento);
          if (dataDesligamento && dataDesligamento < dataInicioJS) return false;
        }
        return true;
      });
    }

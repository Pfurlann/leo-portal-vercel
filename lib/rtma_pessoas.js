    ...
    for (const clube of clubes) {
      try {
        ...
        todasPessoas.forEach(pessoa => {
          ...
          // EXCLUSÃO via posse Lions: 'Apenas LEO/Leão' perdeu status Associado LEO ao virar Lions
          if (pessoa.tipo === 'Apenas LEO/Leão' && pessoa.dataPosseLions && stringValida(pessoa.dataPosseLions)) {
  ...
  try {
    ...
    if (dataPosseLions && dataReferencia >= dataPosseLions) {
      if (dataAssociadoDesde && dataReferencia >= dataAssociadoDesde) {
        // 'Apenas LEO/Leão' no banco: ao virar Lions, perdeu o status de Associado LEO
        if (pessoa.tipo === 'Apenas LEO/Leão') {

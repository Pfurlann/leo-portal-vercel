...
async function portalBuscarAtividades(clubeNome) {
  ...
  const clubesMap = (typeof obterMapaClubesSupabase === 'function') ? await obterMapaClubesSupabase() : null;
  ...
  for (const a of atividades) {
    if (!a.linkFotoOficial && a.id && a.clube && typeof obterUrlFotoPorRegistroId === 'function') {
      const url = await obterUrlFotoPorRegistroId(a.id, a.clube, 'FOTOS_OFICIAIS_ATIVIDADES');
...
async function portalBuscarCampanhas(clubeNome) {
  ...
  for (const c of campanhas) {
    if (!c.linkFotoOficial && c.id && c.clube && typeof obterUrlFotoPorRegistroId === 'function') {
      const url = await obterUrlFotoPorRegistroId(c.id, c.clube, 'FOTOS_OFICIAIS');
...
/**
 * Sanitiza querySuffix: só permite segmentos do tipo order=<col>.<dir>[.nullsfirst|.nullslast].
 * Qualquer outro segmento é silenciosamente descartado.
 * @param {string} raw
 * @returns {string}
 */
function _sanitizeQuerySuffix(raw) {
  if (!raw) return '';
  // Padrão permitido: order=coluna_snake.asc/desc[.nullsfirst/.nullslast]
  const ALLOWED = /^order=[a-z_][a-z0-9_]*\.(asc|desc)(\.nullsfirst|\.nullslast)?$/i;
  const segments = String(raw).replace(/^&/, '').split('&');
  const safe = segments.filter(function(seg) {
    return ALLOWED.test(seg.trim());
  });
 ...
 *   querySuffix é sanitizado: apenas cláusulas order=<col>.<dir> são aceitas; qualquer outra coisa é descartada.
 */
async function portalFetchAllRows(table, querySuffixOrOptions = '') {
  ...
  const rawSuffix = opts.querySuffix ? String(opts.querySuffix) : '';
  const sanitized = _sanitizeQuerySuffix(rawSuffix);
  if (rawSuffix && !sanitized) {
    console.warn('[portalFetchAllRows] querySuffix rejeitado pela lista de permissões — descartado:', rawSuffix);
  }
...
async function portalUpsertCampanha(campanha) {
  ...
  if (!payloadObj.clube_id && typeof obterMapaClubesSupabase === 'function') {
    var clubesMapCampanha = await obterMapaClubesSupabase();
  ...
  try {
    ...
    if (ehPendente && dados.loteId) {
      ...
      if (dados.loteId) {
        ...
        loteAtual = await portalObterLoteAtualEventoReal(dados.eventoId);
        if (!loteAtual) {
...
async function portalValidarCredenciamentoModalidade(inscricaoId, eventoId, modalidadeId) {
  ...
  if (await portalInscricaoJaCredenciadaModalidade(inscricaoId, eventoId, modalidadeId)) return { ok: false, jaCredenciado: true, erro: 'Já credenciado nesta modalidade.', pessoaNome: pessoaNome };
...
async function portalAtualizarInscricoesEnvioLote(eventoId, clubeNome, comprovanteUrl, comprovanteNome, comprovanteTamanho, novoLoteId, permitirExceder) {
  ...
  try {
    ...
    if (loteAntigoId) await portalRecalcularQuantidadeUsadaLote(loteAntigoId);
...
async function portalCriarEixoCampanha(dados) {
  dados = dados || {};
  const al = String(dados.al || '').trim();
  const nome = String(dados.nome || '').trim();
  const tipo = String(dados.tipo || '').trim();
  if (!al || !nome || !['d8', 'dm'].includes(tipo)) {
    throw new Error('Campos al, nome e tipo (d8|dm) são obrigatórios.');
  }
  const table = PORTAL_SUPABASE_TABLES.eixos_campanha;
  const payload = { al, nome, tipo };
  const resp = await portalSupabaseFetch(table, {
    method: 'POST',
    headers: { Prefer: 'return=representation' },
    payload: JSON.stringify([payload])
...
async function portalExcluirEixoCampanha(id) {
  if (!id || !String(id).trim()) throw new Error('ID do eixo não informado.');
  const table = PORTAL_SUPABASE_TABLES.eixos_campanha;

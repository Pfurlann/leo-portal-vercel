...
function portalBuscarTodasAtividades(skipFotoResolution, minimalColumns) {
  const table = PORTAL_SUPABASE_TABLES.atividades;
...
function portalBuscarTodasCampanhas(skipFotoResolution, minimalColumns) {
  const table = PORTAL_SUPABASE_TABLES.campanhas;
...
function portalObterLoteAtualEvento(eventoId) {
...
function portalObterLoteAtualEventoReal(eventoId) {
  const agora = Date.now();
  const lotes = await portalListarLotesEvento(eventoId) || [];
 ...
 */
function portalConsumirRefeicoesInscricaoLote(eventoId, itens) {
 ...
 */
function portalCredenciarInscricoesModalidadeLote(eventoId, itens) {
  if (!eventoId || !Array.isArray(itens) || itens.length === 0) {
    return { processados: 0, sucesso: 0, erros: [] };
  }
  let sucesso = 0;
...
function portalInscricaoJaCredenciadaPlenaria(inscricaoId, eventoId) {
  const modalidadePlenariaId = await portalObterModalidadePlenariaId();
...
function portalEhPastPresidente(clubeNome, pessoaNome) {
...
function portalRemoverCredenciamentoPlenaria(inscricaoId, eventoId) {
  const modalidadePlenariaId = await portalObterModalidadePlenariaId();
...
function portalRelatorioInscritosPorClube(eventoId) {
  const inscricoes = await portalListarInscricoesEvento(eventoId);
  const mapa = {};

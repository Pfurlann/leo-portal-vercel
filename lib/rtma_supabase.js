...
const RTMA_SUPABASE_CONFIG = {
  ...
  serviceRoleKey: process.env.SUPABASE_SERVICE_ROLE_KEY || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJxa3R0YWZsaHRzZGthbWdzY25mIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc2NzE5MDgyMSwiZXhwIjoyMDgyNzY2ODIxfQ.v5Eh3DgTDPnkaptB0wBR_pnafF-j9cX5snv8NdF8q6s'
};
...
async function buscarPessoasDoSupabase(clubeNome) {
  ...
  try {
    ...
    let url = null;
    if (clubeId) {
      url = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/pessoas_rtma?clube_id=eq.${encodeURIComponent(clubeId)}&select=*&order=created_at.asc`;
    } else {
      logWarn(funcao, `Clube não encontrado na tabela clubes (fallback por clube_nome)`, { clubeNome: clubeNomeLimpo });
      url = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/pessoas_rtma?clube_nome=eq.${encodeURIComponent(clubeNomeLimpo)}&select=*&order=created_at.asc`;
    }
    ...
    if (response.getResponseCode() === 200 && !clubeId) {
      try {
        ...
        if (Array.isArray(dadosTmp) && dadosTmp.length === 0) {
          const urlIlike = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/pessoas_rtma?clube_nome=ilike.${encodeURIComponent(clubeNomeLimpo)}&select=*&order=created_at.asc`;
...
async function buscarPessoasMultiplosClubesDoSupabase(clubesNomes) {
  ...
  try {
    ...
    if (ids.length > 0) {
      ...
      baseUrl = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/pessoas_rtma?clube_id=in.(${inList})&select=*&order=clube_nome.asc,created_at.asc`;
    } else {
      const filtros = clubesNomes.map(nome => `clube_nome.eq.${encodeURIComponent(String(nome).trim())}`).join(',');
      baseUrl = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/pessoas_rtma?or=(${filtros})&select=*&order=clube_nome.asc,created_at.asc`;
    }
...
async function buscarUsuarioNoSupabase(email, senha) {
  ...
  try {
    ...
    const url = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/usuarios_acessos?email=eq.${encodeURIComponent(emailLower)}&select=*`;
    const response = await gasStyleFetch(url, {
      method: 'GET',
      headers: {
        'apikey': RTMA_SUPABASE_CONFIG.serviceRoleKey,

...
const { buscarPessoasDoSupabase, buscarPessoasMultiplosClubesDoSupabase, RTMA_USAR_SUPABASE, obterNomesClubesSupabase } = require('./rtma_supabase');
const { logInfo, logWarn, logError, arrayValido, stringValida, respostaSucesso, respostaErro, tratarExcecao, formatarDataRTMA } = require('./rtma_utils');
const { rtmaObterTodosClubes, rtmaClubeExiste, rtmaObterClubesDaRegiao, RTMA_REGIOES } = require('./rtma_config');
const { obterPessoasCache, armazenarPessoasCache, limparCacheClube } = require('./rtma_cache');
 ...
 */
async function buscarPessoasRTMA(clube, tipo = null, formacao = null, profissao = null) {
  ...
  try {
    ...
    let pessoasDados = [];
    if (typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true) {
      pessoasDados = await buscarPessoasDoSupabase(clube);
    } else {
 ...
 */
async function buscarPessoasDistrito(filtros) {
  ...
  try {
    ...
    const clubesParaBuscar = await determinarClubesParaBuscar(usuario, clube, regiao);
    ...
    if (typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true && clubesParaBuscar.length > 1) {
      ...
      try {
        const pessoasBatch = await buscarPessoasMultiplosClubesDoSupabase(clubesParaBuscar);
        ...
        for (const nomeClube of clubesParaBuscar) {
          try {
            const pessoas = await buscarPessoasRTMAOtimizado(nomeClube, tipo, formacao, profissao);
...
/**
 * Determinar quais clubes buscar baseado no usuário.
 * Para distrito, a fonte principal é a tabela `clubes` (Supabase); RTMA_REGIOES é fallback.
 * @param {Object} usuario - Dados do usuário
 * @param {string} clube - Clube específico (opcional)
 * @param {string} regiao - Região específica (opcional)
 * @return {Promise<Array>} Lista de clubes para buscar
 */
async function determinarClubesParaBuscar(usuario, clube, regiao) {
  ...
  if (usuario.isDistrito) {
    ...
    if (Array.isArray(regiao) && regiao.length > 0) {
      let clubes = [];
      for (const r of regiao) {
        const daTabela = (typeof obterNomesClubesSupabase === 'function')
          ? await obterNomesClubesSupabase('Ativo', r)
          : [];
        if (arrayValido(daTabela) && daTabela.length > 0) {
          clubes = clubes.concat(daTabela);
        } else {
          clubes = clubes.concat(rtmaObterClubesDaRegiao(r));
        }
      }
      console.log('🔍 [rtma_pessoas.js] Retornando clubes de múltiplas regiões:', clubes.length);
      return [...new Set(clubes)];
    }
    if (stringValida(regiao)) {
      let clubesRegiao = (typeof obterNomesClubesSupabase === 'function')
        ? await obterNomesClubesSupabase('Ativo', regiao)
        : [];
      if (!arrayValido(clubesRegiao) || clubesRegiao.length === 0) {
        clubesRegiao = rtmaObterClubesDaRegiao(regiao);
      }
    ...
    // Todos os clubes ativos da tabela `clubes`
    console.log('🔍 [rtma_pessoas.js] Buscando todos os clubes (tabela clubes)...');
    let todosClubes = (typeof obterNomesClubesSupabase === 'function')
      ? await obterNomesClubesSupabase('Ativo')
      : [];
    if (arrayValido(todosClubes) && todosClubes.length > 0) {
      console.log('✅ [rtma_pessoas.js] obterNomesClubesSupabase retornou:', todosClubes.length, 'clubes');
      return todosClubes;
    }
    if (typeof rtmaObterTodosClubes === 'function') {
      todosClubes = rtmaObterTodosClubes();
      console.log('✅ [rtma_pessoas.js] Fallback rtmaObterTodosClubes:', todosClubes ? todosClubes.length : 0, 'clubes');
      return todosClubes || [];
    }
    todosClubes = [];
    if (typeof RTMA_REGIOES !== 'undefined') {
      ...
      console.log('✅ [rtma_pessoas.js] Fallback RTMA_REGIOES:', todosClubes.length, 'clubes');
    } else {
 ...
 * @return {Array} Array de pessoas
 */
async function buscarPessoasRTMAOtimizado(clube, tipo, formacao, profissao) {
  try {
...
async function criarPessoaRTMA(clube, dados) {
  ...
  try {
    ...
    if (typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true) {
      const resp = await criarPessoaNoSupabase(clube, dados);
 ...
 * @return {Object} Resultado da operação
 */
async function editarPessoaRTMA(clube, pessoaId, dados) {
  ...
  try {
    ...
    if ((typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true) || isUUID) {
      return await editarPessoaNoSupabase(clube, pessoaId, dados);
    }
 ...
 * @return {Object} Resultado da operação
 */
async function desligarPessoaRTMA(clube, pessoaId, dataDesligamento) {
  ...
  try {
    ...
    // Usar Supabase se configurado e se o ID for UUID
    if ((typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true) || isUUID) {
      return await desligarPessoaNoSupabase(clube, pessoaId, dataDesligamento);
    }
...
async function buscarOpcoesFiltros_rtma_pessoas(clube) {
  ...
  try {
    ...
    // Buscar da planilha
    const opcoes = await extrairOpcoesFiltrosDaPlanilha(clube);
...
async function buscarOpcoesFiltrosDistritoPessoas_rtma_pessoas(usuario) {
  ...
  try {
    ...
    if (usuario.isRegiao && usuario.clubesPermitidos) {
      ...
      // Obter todos os clubes ativos da tabela `clubes`
      clubesPermitidos = (typeof obterNomesClubesSupabase === 'function')
        ? await obterNomesClubesSupabase('Ativo')
        : [];
      if (!arrayValido(clubesPermitidos) || clubesPermitidos.length === 0) {
 ...
 * @return {Object} Opções de filtros
 */
async function extrairOpcoesFiltrosDaPlanilha(clube) {
  ...
  try {
    ...
    const pessoas = await buscarPessoasRTMA(clube);
 ...
 */
async function coletarOpcoesFiltrosMultiplosClubes(clubes) {
  const opcoes = {
    clubes: clubes.sort(),
    formacoes: new Set(),
    profissoes: new Set()
  ...
  for (let i = 0; i < clubes.length; i += batchSize) {
    ...
    for (const clube of batch) {
      try {
        const opcoesCLube = await extrairOpcoesFiltrosDaPlanilha(clube);
 ...
 */
async function aplicarCargoNominataNasPessoasRelatorio(pessoas) {
  ...
  let linhas = [];
  try {
    linhas = typeof listarCargosNominataPorAL === 'function' ? (await listarCargosNominataPorAL(alNom)) : [];
  } catch (e) {
...
// Tipos de relatório liberados para quem não é acesso distrital (clube/região).
// O distrito continua com todos os tipos.
const RTMA_TIPOS_RELATORIO_NAO_DISTRITAL = ['atual', 'rtma_detalhado'];

/**
 * Clubes que o usuário pode enxergar no relatório de pessoas.
 * Distrito: todos. Região: os clubes da região. Clube: apenas o próprio.
 * @param {Object} usuario - Dados do usuário
 * @param {Object} config - Configuração do RTMA
 * @return {Array<string>} Lista de clubes permitidos
 */
function rtmaClubesDoEscopoRelatorio(usuario, config) {
  if (usuario && usuario.isDistrito) {
    return typeof rtmaObterListaClubesParaRelatorio === 'function'
      ? rtmaObterListaClubesParaRelatorio(config)
      : Object.keys(config.planilhasClubes || {});
  }

  if (usuario && usuario.isRegiao && Array.isArray(usuario.clubesPermitidos) && usuario.clubesPermitidos.length > 0) {
    return usuario.clubesPermitidos.slice();
  }

  return (usuario && usuario.clube) ? [usuario.clube] : [];
}

/**
 * Gerar relatório Excel com total de pessoas por clube
 * Distrito enxerga todos os clubes; região e clube ficam restritos ao próprio escopo.
 * @param {Object} usuario - Dados do usuário
 ...
 */
async function gerarRelatorioExcelDistrito(usuario, dataReferencia = null, dataInicioPeriodo = null, dataFimPeriodo = null, incluirInativos = false, dadosFiltrados = null, tipoRelatorioParam = null) {
  const funcao = 'gerarRelatorioExcelDistrito';

  try {
    logInfo(funcao, 'Iniciando geração de relatório Excel de pessoas', {
      dataReferencia, dataInicioPeriodo, dataFimPeriodo, temDadosFiltrados: !!dadosFiltrados
    });

    const isDistrito = !!(usuario && usuario.isDistrito);
    if (!usuario || (!isDistrito && !stringValida(usuario.clube))) {
      return respostaErro('Acesso negado. Faça login novamente para gerar este relatório.');
    }

    const config = getRtmaConfig();
    const clubesPermitidos = rtmaClubesDoEscopoRelatorio(usuario, config);
    if (clubesPermitidos.length === 0) {
      return respostaErro('Nenhum clube disponível para este usuário.');
    }
    const escopoLabel = isDistrito ? 'Distrito LEO LD-8' : clubesPermitidos.join(', ');
    ...
    // Barrar tipo não liberado antes de validar datas, para a mensagem ser a certa
    if (!isDistrito && RTMA_TIPOS_RELATORIO_NAO_DISTRITAL.indexOf(tipoRelatorio) === -1) {
      return respostaErro('Este relatório está disponível apenas para o acesso distrital.');
    }
    ...
    // Fora do distrito, só os tipos liberados (o front já esconde os demais)
    if (!isDistrito && RTMA_TIPOS_RELATORIO_NAO_DISTRITAL.indexOf(tipoRelatorio) === -1) {
      return respostaErro('Este relatório está disponível apenas para o acesso distrital.');
    }
    ...
    switch (tipoRelatorio) {
      case 'data':
        tituloBase = `Relatório de Pessoas por Clube em ${dataReferencia} - ${escopoLabel}`;
        break;
      case 'periodo':
        tituloBase = `Relatório de Pessoas por Clube - Período ${dataInicioPeriodo} a ${dataFimPeriodo} - ${escopoLabel}`;
        break;
      case 'detalhado':
        tituloBase = `RTMA Detalhado - ${escopoLabel} em ${dataReferencia}`;
        break;
      case 'rtma_detalhado':
        tituloBase = `RTMA Detalhado - Todas as Colunas - ${escopoLabel}`;
        break;
      case 'associados_master_senior':
        tituloBase = `Associados LEO ativos — classificação Master/Senior na data ${dataReferencia || ''} — ${escopoLabel}`;
        break;
      default:
        tituloBase = `Relatório de Pessoas Ativas por Clube - ${escopoLabel}`;
    }
    ...
    if (tipoRelatorio === 'resumo_dm') {
      ...
      // Reaplicar o escopo do usuário: o front nunca é a única barreira de permissão.
      const pessoasDoEscopo = isDistrito
        ? dadosFiltrados
        : dadosFiltrados.filter(pessoa => pessoa && clubesPermitidos.indexOf(pessoa.clube) !== -1);
      logInfo(funcao, `Usando dados já filtrados do frontend: ${pessoasDoEscopo.length} pessoas`);

      // Agrupar pessoas por clube
      const pessoasPorClube = {};
      pessoasDoEscopo.forEach(pessoa => {
      ...
      const pessoasUnificadas = pessoasDoEscopo.map(function (pessoa) {
    ...
    if (tipoRelatorio === 'resumo_dm') {
      ...
      const resumoDM = await calcularResumoTrimestralDM(dataInicioPeriodo, dataFimPeriodo);
    ...
    if (dadosRelatorio.pessoas && dadosRelatorio.pessoas.length > 0) {
      ...
      await aplicarCargoNominataNasPessoasRelatorio(dadosRelatorio.pessoas);
    }
 ...
 */
async function gerarExcelDetalhado(usuario, dataInicioPeriodo, dataFimPeriodo, clubeFiltrado = null) {
  ...
  try {
    ...
    const statusClubePorNome = (typeof obterMapaStatusClubesSupabase === 'function')
      ? await obterMapaStatusClubesSupabase()
      : {};
    ...
    if (clubeFiltrado) {
      ...
      // Se é acesso distrital, buscar todos os clubes ativos da tabela `clubes`
      clubesParaBuscar = (typeof obterNomesClubesSupabase === 'function')
        ? await obterNomesClubesSupabase('Ativo')
        : [];
      if (!arrayValido(clubesParaBuscar) || clubesParaBuscar.length === 0) {
    ...
    if (clubesParaBuscar.length > 1 && typeof buscarPessoasMultiplosClubesDoSupabase === 'function') {
      try {
        todasPessoasRaw = await buscarPessoasMultiplosClubesDoSupabase(clubesParaBuscar);
        ...
        for (const clube of clubesParaBuscar) {
          try {
            const pessoasClube = await buscarPessoasDoSupabase(clube);
    ...
    const obterDestaqueMovimentacaoDM = function(pessoa) {
      ...
      // Usar associadoDesde para detectar inclusão (consistente com calcularResumoDMComFiltrosDashboard)
      let dataAssociadoLEO = null;
      if (pessoa.associadoDesde && stringValida(pessoa.associadoDesde)) {
        dataAssociadoLEO = converterDataBRParaJS(pessoa.associadoDesde);
      }
      ...
      const foiIncluidoNoPeriodo = dataAssociadoLEO &&
        dataAssociadoLEO >= dataInicioObj && dataAssociadoLEO <= dataFimObj;
      let inclusao = false;
      if (foiIncluidoNoPeriodo) {
        const tipoHistoricoInclusao = obterTipoHistoricoPessoa(pessoa, dataAssociadoLEO, false);
    ...
    if (typeof aplicarCargoNominataNasPessoasRelatorio === 'function') {
      await aplicarCargoNominataNasPessoasRelatorio(todasPessoas);
    }
    ...
    const resumoDM = await calcularResumoDMComFiltrosDashboard(dataInicioPeriodo, dataFimPeriodo);
 ...
 */
async function gerarRelatorioMovimentacaoDM(usuario, dataInicioPeriodo, dataFimPeriodo) {
  ...
  try {
    ...
    const config = getRtmaConfig();
    // Obter clubes ativos da tabela `clubes` (fallback: RTMA_REGIOES)
    let clubes = (typeof obterNomesClubesSupabase === 'function')
      ? await obterNomesClubesSupabase('Ativo')
      : [];
    if (!arrayValido(clubes) || clubes.length === 0) {
      clubes = (typeof rtmaObterTodosClubes === 'function') 
        ? rtmaObterTodosClubes() 
        : (() => {
            const todosClubes = [];
            if (config.regioes) {
    ...
    for (const clube of clubes) {
      try {
        ...
        todasPessoas.forEach(pessoa => {
          // Data de posse genérica (para totalFim)
          const dataPosseClube = obterDataPosseClube(pessoa);
          if (!dataPosseClube) {
            return; // Sem data de posse, não pode ser contabilizado
          }

          // Data em que se tornou Associado LEO (base para totalInicio e inclusoes)
          let dataAssociadoLEO = null;
          if (pessoa.associadoDesde && stringValida(pessoa.associadoDesde)) {
            dataAssociadoLEO = converterDataBRParaJS(pessoa.associadoDesde);
          }
          ...
          // TOTAL INÍCIO: quem era Associado LEO ANTES do início do período
          // Usa associadoDesde < dataInicioObj (estrito) para não sobrepor com inclusoes
          if (eraAssociadoLEONoInicio) {
            const tinhaPosseNoInicio = dataAssociadoLEO && dataAssociadoLEO < dataInicioObj;
          ...
          // INCLUSÕES: tornou-se Associado LEO DENTRO do período (associadoDesde >= inicio && <= fim)
          // Usa associadoDesde direto para capturar Pré-LEOs que viraram Associados no período
          const foiIncluidoNoPeriodo = dataAssociadoLEO &&
            dataAssociadoLEO >= dataInicioObj && dataAssociadoLEO <= dataFimObj;
          if (foiIncluidoNoPeriodo) {
            const tipoHistoricoInclusao = obterTipoHistoricoPessoa(pessoa, dataAssociadoLEO, false);
 ...
 */
async function calcularResumoDMComFiltrosDashboard(dataInicioPeriodo, dataFimPeriodo) {
  ...
  try {
    ...
    // Obter clubes: tabela `clubes` primeiro; RTMA_REGIOES como fallback
    let clubes = [];
    
    // Estratégia 1: tabela clubes (Supabase)
    if (typeof obterNomesClubesSupabase === 'function') {
      clubes = await obterNomesClubesSupabase('Ativo');
      logInfo(funcao, 'Clubes obtidos via obterNomesClubesSupabase', { total: clubes ? clubes.length : 0 });
    }
    
    // Estratégia 2: lista hardcoded RTMA_REGIOES
    if (!arrayValido(clubes) || clubes.length === 0) {
      if (typeof rtmaObterTodosClubes === 'function') {
    ...
    // Estratégia 3: Se não obteve clubes, tentar via config
    if (!arrayValido(clubes) || clubes.length === 0) {
    const config = getRtmaConfig();
    ...
    // Estratégia 4: Fallback direto para RTMA_REGIOES
    if (!arrayValido(clubes) || clubes.length === 0) {
    ...
    for (const clube of clubes) {
      try {
        ...
        dadosPorClube.push({ clube, regiao: regiaoClube, totalInicio: 0, inclusoes: 0, exclusoes: 0, totalFim: 0 });
      }
    }
 ...
 */
async function calcularResumoTrimestralDM(dataInicioPeriodo, dataFimPeriodo) {
...
function aplicarFiltrosHistoricos(pessoas, dataReferencia, incluirInativos) {
  ...
  try {
    ...
    pessoas.forEach(pessoa => {
      ...
      const tipoHistorico = obterTipoHistoricoPessoa(pessoa, dataRef, incluirInativos);
      if (tipoHistorico) {
    ...
    const totalInativos = pessoas.filter(p => p.status !== 'Ativo').length;
    const inativosIncluidos = pessoasFiltradas.filter(p => p.status !== 'Ativo').length;
    console.log(`[aplicarFiltrosHistoricos] dataRef=${dataReferencia} incluirInativos=${incluirInativos} total=${pessoas.length} inativos_entrada=${totalInativos} filtrado=${pessoasFiltradas.length} inativos_saida=${inativosIncluidos}`);
  ...
  try {
    ...
    if (dataPosseLions && dataReferencia >= dataPosseLions) {
      if (dataAssociadoDesde && dataReferencia >= dataAssociadoDesde) {
        // tipo normalizado 'Associado LEO/Leão' = perdeu status Associado LEO ao virar Lions
        if (pessoa.tipo === 'Associado LEO/Leão') {
    ...
    // Segundo: posse no Clube na data de referência
      ...
      // "LEO/Leão" exige posse no Lions; sem ela, completar 31 anos = egresso por idade (inativo)
      tipoCalculado = completou31Anos ? null : 'Associado LEO';
    }
    // Terceiro: Pré-LEO
    else if (dataInicioPreLeo && dataReferencia >= dataInicioPreLeo) {
      if (!dataAssociadoDesde || dataReferencia < dataAssociadoDesde) {
        // Pré-LEO que completa 31 sem posse no Lions também é egresso por idade
        tipoCalculado = completou31Anos ? null : 'Pré LEO';
      }
    }
    ...
    // "Associado LEO/Leão" histórico: bypassa check de atividade porque a pessoa
    // transitou para Lions (outra organização) — o conceito de "ativo no LEO" não se aplica.
    // Usa APENAS o tipoCalculado (baseado em datas), nunca pessoa.tipo (tipo atual do banco),
    // para não classificar como LEO/Leão em períodos anteriores à posse no Lions.
    if (tipoCalculado === 'Associado LEO/Leão') {
 ...
 * @return {Array} Array de pessoas filtradas
 */
async function buscarPessoasComFiltroData(clube, dataReferencia = null, mostrarInativos = false, tipo = null, formacao = null, profissao = null) {
  ...
  try {
    ...
    let pessoas = await buscarPessoasRTMA(clube);
 ...
 * @return {Array} Array de pessoas filtradas
 */
async function buscarPessoasComFiltroPeriodo(clube, dataInicioPeriodo, dataFimPeriodo, mostrarInativos = false, tipo = null, formacao = null, profissao = null) {
 ...
 * @return {Array} Array de pessoas de múltiplos clubes
 */
async function buscarPessoasDistritoComFiltroData(filtros) {
  ...
  try {
    ...
    if (typeof RTMA_USAR_SUPABASE !== 'undefined' && RTMA_USAR_SUPABASE === true && clubesParaBuscar.length > 1) {
      try {
        ...
        for (const nomeClube of clubesParaBuscar) {
          try {
            let pessoas = [];
            if (dataInicioPeriodo && dataFimPeriodo && stringValida(dataInicioPeriodo) && stringValida(dataFimPeriodo)) {
              pessoas = await buscarPessoasComFiltroPeriodo(nomeClube, dataInicioPeriodo, dataFimPeriodo, mostrarInativos, tipo, formacao, profissao);
            } else {
              pessoas = await buscarPessoasComFiltroData(nomeClube, dataReferencia, mostrarInativos, tipo, formacao, profissao);
            }
            if (arrayValido(pessoas)) {
              todasPessoas = todasPessoas.concat(pessoas.map(p => ({ ...p, clube: nomeClube })));
            }
          } catch (error) {
            logWarn(funcao, `Erro ao processar clube ${nomeClube}`, error.message);
          }
      ...
      // Processar clubes sequencialmente (funções são async)
      for (const nomeClube of clubesParaBuscar) {
        try {
          let pessoas = [];
          if (dataInicioPeriodo && dataFimPeriodo && stringValida(dataInicioPeriodo) && stringValida(dataFimPeriodo)) {
 ...
 * @return {Object} Resultado da operação
 */
async function gerarExcelDM(usuario, dataInicioPeriodo, dataFimPeriodo) {
  ...
  try {
    ...
    const dadosRelatorio = await calcularResumoDMComFiltrosDashboard(dataInicioPeriodo, dataFimPeriodo);
    ...
    if (clubesParaBuscar.length > 1 && typeof buscarPessoasMultiplosClubesDoSupabase === 'function') {
      try {
        pessoasRaw = await buscarPessoasMultiplosClubesDoSupabase(clubesParaBuscar);
      } catch (erroBatch) {
        logWarn(funcao, 'Erro ao buscar pessoas em batch, tentando individualmente', erroBatch);
        for (const clube of clubesParaBuscar) {
          try {
            const pessoasClube = await buscarPessoasRTMA(clube);
    ...
    if (typeof aplicarCargoNominataNasPessoasRelatorio === 'function') {
      await aplicarCargoNominataNasPessoasRelatorio(todasPessoasDm);
    }
    ...
    try {
      await salvarRelatorioDMNoSupabase(dadosRelatorio, usuario);
    } catch (errorSupabase) {

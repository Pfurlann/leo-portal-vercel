...
const { buscarPessoasDoSupabase, buscarPessoasMultiplosClubesDoSupabase, RTMA_USAR_SUPABASE } = require('./rtma_supabase');
    ...
    for (const clube of clubes) {
      try {
        ...
        todasPessoas.forEach(pessoa => {
          ...
          // EXCLUSÃO via posse Lions: tipo 'Associado LEO/Leão' perdeu status Associado LEO ao virar Lions
          if (pessoa.tipo === 'Associado LEO/Leão' && pessoa.dataPosseLions && stringValida(pessoa.dataPosseLions)) {
            const dataPosseLionsObj = converterDataBRParaJS(pessoa.dataPosseLions);
            if (dataPosseLionsObj && dataPosseLionsObj >= dataInicioObj && dataPosseLionsObj <= dataFimObj) {
              const diaAntesPosseLions = new Date(dataPosseLionsObj);
              diaAntesPosseLions.setDate(diaAntesPosseLions.getDate() - 1);
              const tipoAntesPosseLions = obterTipoHistoricoPessoa(pessoa, diaAntesPosseLions, false);
              if (tipoAntesPosseLions === 'Associado LEO' || tipoAntesPosseLions === 'Associado LEO e LEO/Leão') {
                dadosClube.exclusoes++;
              }
            }
          }

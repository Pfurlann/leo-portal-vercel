...
/**
 * Nomes dos clubes cadastrados na tabela `clubes` (fonte do cadastro de clubes).
 * @param {string} [filtroStatus='Ativo'] - Status a filtrar; string vazia = todos
 * @param {string} [filtroRegiao] - Região A/B/D (opcional)
 * @return {Promise<string[]>} Nomes ordenados (pt-BR), sem duplicatas
 */
async function obterNomesClubesSupabase(filtroStatus, filtroRegiao) {
  const funcao = 'obterNomesClubesSupabase';
  try {
    let url = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/clubes?select=nome,status,regiao&order=nome.asc`;
    const response = await gasStyleFetch(url, {
      method: 'GET',
      headers: {
        'apikey': RTMA_SUPABASE_CONFIG.serviceRoleKey,
    ...
    if (response.getResponseCode() !== 200) {
      // Fallback sem colunas status/regiao (schemas antigos)
      url = `${RTMA_SUPABASE_CONFIG.url}/rest/v1/clubes?select=nome&order=nome.asc`;
      const response2 = await gasStyleFetch(url, {
        method: 'GET',
        headers: {
          'apikey': RTMA_SUPABASE_CONFIG.serviceRoleKey,
          'Authorization': `Bearer ${RTMA_SUPABASE_CONFIG.serviceRoleKey}`,
      ...
      if (response2.getResponseCode() !== 200) {
        logError(funcao, 'Erro ao listar nomes de clubes', response2.getContentText());
        return [];
      }
      const raw2 = JSON.parse(response2.getContentText() || '[]');
      const nomes2 = [];
      const seen2 = {};
      (raw2 || []).forEach(function (c) {
        const nome = c && c.nome ? String(c.nome).trim() : '';
        if (!nome || seen2[nome]) return;
        seen2[nome] = true;
        nomes2.push(nome);
      });
      return nomes2;
    ...
    const statusFiltro = (filtroStatus === undefined || filtroStatus === null)
      ? 'Ativo'
      : String(filtroStatus).trim();
    const regiaoFiltro = filtroRegiao ? String(filtroRegiao).trim() : '';
    const nomes = [];
    const seen = {};
    (raw || []).forEach(function (c) {
      const nome = c && c.nome ? String(c.nome).trim() : '';
      if (!nome || seen[nome]) return;
      const status = (c && c.status && String(c.status).trim()) ? String(c.status).trim() : 'Ativo';
      if (statusFiltro && status !== statusFiltro) return;
      const regiao = (c && c.regiao && String(c.regiao).trim()) ? String(c.regiao).trim() : '';
      if (regiaoFiltro && regiao !== regiaoFiltro) return;
      seen[nome] = true;
      nomes.push(nome);
    });
    nomes.sort(function (a, b) {
      return a.localeCompare(b, 'pt-BR', { sensitivity: 'base', ignorePunctuation: true, numeric: true });
    });
    logInfo(funcao, `Nomes de clubes obtidos: ${nomes.length}`, {
      status: statusFiltro || '(todos)',
      regiao: regiaoFiltro || '(todas)'
    });
    return nomes;
  } catch (error) {
    logError(funcao, 'Erro ao obter nomes de clubes', error.message);
    return [];
  }
}
